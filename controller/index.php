    ...
    if (preg_match('/^\-\-[a-f0-9]+\-A\-\-$/i', trim($BODY[$line]))) {
        ...
        while ( $line < $BodySize ) {
            ...
            trim($BODY[$line]), $matchesA)) {
					...
					print "matchesA1--->".$matchesA[1]."\n";
					print "matchesA2--->".$matchesA[2]."\n";
					print "matchesA3--->".$matchesA[3]."\n";				print "matchesA4--->".$matchesA[4]."\n";
					print "matchesA5--->".$matchesA[5]."\n";
					print "matchesA6--->".$matchesA[6]."\n";				print "matchesA7--->".$matchesA[7]."\n";
					print "matchesA8--->".$matchesA[8]."\n";
					print "matchesA9--->".$matchesA[9]."\n";				print "matchesA10--->".$matchesA[10]."\n";					
...
//[19/Sep/2018:05:39:05 --0400] AcADA7AcTcAcApAcdc5cAcpc 192.168.1.10 64895 127.0.0.1 80
                    $PhaseA['Year']       = $matchesA[3];
                    $PhaseA['Hour']       = $matchesA[4];
                    $PhaseA['Timestamp']  = $matchesA[3]."-".$PhaseA['Month']."-".$matchesA[1]." ".$matchesA[4];
                    $PhaseA['Timezone']   = $matchesA[5];//--0400
                    $PhaseA['Date']       = $matchesA[3]."-".$PhaseA['Month']."-".$matchesA[1];
                    $PhaseA['UniqID']     = $matchesA[6];//AckcAcAc5ctkOcCcAcAcAcKc
                    $PhaseA['ClientIP']   = $matchesA[7];//192.168.1.10
                    $PhaseA['SourcePort'] = $matchesA[8];//52261
                    $PhaseA['ServerIP']   = $matchesA[9];//127.0.0.l
                    $PhaseA['ServerPort'] = $matchesA[10];//80
                }
    ...
    if (preg_match('/^\-\-[a-f0-9]+\-F\-\-$/i', trim($BODY[$line]))) {
        ...
        while ( $line < $BodySize ) {
            if (preg_match('/^\-\-[a-f0-9]+\-[ABCEHIKZ]\-\-$/i', trim($BODY[$line]))) {
				...
				//'/^(HTTP\/\d\.\d)\s(\d\d\d)\s([\w\s]+)/i'
                if (preg_match('/^(HTTP\/\d\.\d)\s(\d\d\d)/i', trim($BODY[$line]), $matchesF)) {
                    $PhaseF['Protocol'] = $matchesF[1];
                    $PhaseF['Status']   = $matchesF[2];
                    $PhaseF['MSG']      = $matchesF[3];
					print "matchesF1-3-->".$matchesF[1]."->".$matchesF[2]."->".$matchesF[3]."\r\n";
                ...
                if (preg_match('/^Message:\s/i', $currentHLine)) {
                    ...
                    while (true){
                        ...
                        if ($message_Ruleid) {
							...
							print "msg_ruleid-->$message_Ruleid\r\n";
                            continue;
                        }
                        ...
                        if ($message_severity) {
							...
							print "msg_severity-->$message_severity\r\n";
                            continue;
                        }
...
try {
	...
	print "clentipcc--->".$ClientIPCC."\n";
	//$ClientIPCC='CN';
	//$PhaseF['Status']=200;
	if($PhaseF['Content-Length']==''){
		$PhaseF['Content-Length']=0;}
    if($PhaseH['Apache_error-Line']==''){
		$PhaseH['Apache_error-Line']=0;}
	//$PhaseH['Apache_error-Line']=403;
    if($PhaseH['Apache_error-Level']==''){
		$PhaseH['Apache_error-Level']=0;}	
	//$PhaseH['Apache_error-Level']=3;
    if($PhaseH['Score']['In_Total']==''){
		$PhaseH['Score']['In_Total']==0;}

    if($PhaseH['Score']['In_SQLi']==''){
		$PhaseH['Score']['In_SQLi']==0;}
		
	if($PhaseH['Score']['In_XSS']==''){
		$PhaseH['Score']['In_XSS']==0;}
	//$PhaseH['Score']['In_Total']=333;
	//$PhaseH['Score']['In_SQLi']=2;
	//$PhaseH['Score']['In_XSS']=2;
